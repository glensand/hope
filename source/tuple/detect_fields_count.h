...
namespace hope {
    namespace detail {
        ...
        constexpr auto is_constructible_n(const T&, std::index_sequence<Is...>)
            ->decltype(T{ any_convertible<Is>{}... }, bool()) {
            return true;
        }

        template <typename T, std::size_t... Is>
        constexpr auto is_constructible_n(const T&, ...) {
            return false;
        }

        template <typename T, std::size_t... Is>
        constexpr std::size_t detect_fields_count_impl(const T& object, std::index_sequence<Is...> sequence) {
            bool bs[] = { is_constructible_n(object, std::make_index_sequence<Is>())... };
